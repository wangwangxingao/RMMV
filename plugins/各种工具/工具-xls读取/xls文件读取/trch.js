...
ww.trch._dataName = ['$dataActors', '$dataClasses', '$dataSkills', '$dataItems', '$dataWeapons', '$dataArmors', '$dataEnemies', '$dataTroops', '$dataStates', '$dataAnimations', '$dataTilesets', '$dataCommonEvents', '$dataSystem', '$dataMapInfos']
...
ww.trch.changeLanguageOther = function (language) {
    ...
    for (var i = 0; i < ns.length; i++) {
        var n = ns[i]
        ww.trch.changeLanguageData(n, language)
    }
}
        ...
        if ($dataMap) {
            if ($gameMap && $gameMap.mapId()) {
                ...
                if (ww.trch._data[mapid]) {
                    ww.data2xls.copyl2o(ww.trch[mapid], $dataMap, 0, language)
                }
            }
        }
    } else {
        if (window[name] && ww.trch._data[name]) {
            ww.data2xls.copyl2o(ww.trch._data[name], window[name], 0, language)
        }
    }
} 
