 ...
 *    
 * Decrypter.encrypt( )  加密所有需要加秘的文件 (设置上true的项目)
 * Decrypter.encryptDir(dir )  加密所有需要加秘的文件夹 dir 文件夹名
 * Decrypter.encryptFile(dirpath, name, type )  加密需要加秘的文件 dirpath 文件夹地址 name 名称 type 深度
 *  
 *
 *
 */
...
Decrypter.encryptDir = function (dir, nosync) {
	...
	this.encryptFile(dirpath, dir, Infinity, nosync)
}

Decrypter.encryptFile = function (dirpath, name, type, nosync) {
	if (!Utils.isNwjs()) {
		console.log("not is Nwjs")
		return {}
	}
	...
	var loadDirList = function (url, type) {
		...
		if (nosync[2]) {
			fs.readdir(url, fsreaddir)
		} else {
			var files = fs.readdirSync(url)
			fsreaddir(null, files)
		}
	}
	var encryptSave = function (url) {
		...
		var saveEncryptFile = function (url, data) {
			...
			var fswriteFile = function () {
				Decrypter.encryptfilelist.end(url, "file saveed")
			}
			if (nosync[3]) {
				fs.writeFile(url, data, fswriteFile);
			} else {
...
Decrypter.get = function () {
	...
	var parameters = PluginManager._parameters['Decrypter'];
	if (parameters) {
	} else {
		var pls = PluginManager._parameters
		for (var n in pls) {
			if (pls[n] && ("Decrypter" in pls[n])) {
				parameters = pls[n]
			}
		}
	}
	parameters = parameters || {}
	var miyao = parameters["miyao"]
	var audio = parse( parameters["audio"]) || false
	var img = parse( parameters["img"]) || false
	var data = parse( parameters["data"]) || false
