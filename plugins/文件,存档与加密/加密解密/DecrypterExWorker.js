//=============================================================================
// DecrypterExWorker.js
//=============================================================================
/*:
 * @plugindesc 仿mv加密的worker , 状态请设为off
 * @author wangwang
 *
 * @param DecrypterExWorker
 * @desc 仿mv加密的worker,状态请设为off
 * @default 1.0 
 * @help 
 * 仿mv加密的worker , 状态请设为off 
 * 
 *
 */
...
getRe = function (event) {
    var data = event.data
    var type = data.type

    var headerlength = data.headerlength
    var arrayBuffer = data.data
    var encryptionKey = data.key
    var url = data.url 
    var rety = data.rety 
    var da 
    if (type == "arrayBuffer") { 
        var da = encryptArrayBuffer(  headerlength, arrayBuffer, encryptionKey)
    } else if (type == "text") { 
        var da = encryptText(  headerlength, arrayBuffer, encryptionKey)
    }
    var re = { url: url, data: da, type: rety }
    return re
}

encryptArrayBuffer = function ( headerlength, arrayBuffer, encryptionKey) { 
    if (!arrayBuffer) {
...
encryptArrayBufferEX = function(arrayBuffer) {
...
encryptText = function (hearderlength, data, encryptionKey) { 
    if (!data) {
        data = ""
    }
