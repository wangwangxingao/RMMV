 ...
 * @param  Knapsack
 * @desc 插件 背包 ,作者:汪汪
 * @default 汪汪,TouchEx,TouchButton
 * 
 * 
 * @param switch
 * @desc 背包开关,
...
Sprite_Knapsack.prototype.getInit = function() {
    ...
    this._switch = Sprite_Knapsack._switch
...
/**背包开关 */
Sprite_Knapsack.switch = function(i) {
    if (i === undefined) {
        if (this._switch) {
...
Sprite_Knapsack.prototype.updateSwitch = function() {
    if (this._hideshow == "show" || this._hideshow == "showed") {
        if (Sprite_Knapsack.switch() == 2) {
            this.hide()
        }
    } else {
        if (Sprite_Knapsack.switch() == 1) {
            this.show()
        }
    }
};
