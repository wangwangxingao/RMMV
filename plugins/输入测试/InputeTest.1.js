...
KeyBoard.prototype.keyBase = function(i, z, x, y) {
    console.log(i, z, x, y)
    if (!this._keys[i]) {
        this._keys[i] = new KeyInput()
    }
    this._keys[i].keyChange(z, this.time(), x, y)
};

/**全部按键 */
KeyBoard.prototype.keyAll = function(x, y) {
    var z = Object.getOwnPropertyNames(this._allKey)
...
KeyBoard.prototype.keyMoveBase = function(i, x, y) {
    ...
    this._keys[i].keyMove(this.time(), x, y)
};
/**键移动 */
...
/**长时间 */
KeyInput.prototype.keyisLong = function(time) {
...
Input._onKeyDown = function(event) {
    ...
    //当前状态 键 =true
    this._allKey.keyDown(event.keyCode)
};
...
Input._onKeyUp = function(event) {
    this._allKey.keyUp(event.keyCode)
        // 如果 键值===0
